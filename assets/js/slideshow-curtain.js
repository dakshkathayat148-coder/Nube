...
(function () {
   ...
   const navigateTo = (targetIdx) => {
    ...
    /* Wheel — transition slides while wrapper is at viewport top;
       release at last slide (fwd) / first slide (bwd) so page scrolls naturally. */
    window.addEventListener(
        "wheel",
        (e) => {
            const rect = wrapper.getBoundingClientRect();
            /* Only intercept when wrapper is near the top of the viewport */
            if (Math.abs(rect.top) > 10 || rect.bottom < 0) return;
            e.preventDefault();
            const raw =
            ...
            const isLast = wrapIndex(currentIndex, projectData.length) === projectData.length - 1;
            const isFirst = wrapIndex(currentIndex, projectData.length) === 0;
            /* Release at boundaries so page can scroll normally */
            if ((raw > 0 && isLast) || (raw < 0 && isFirst)) return;
   ...
   window.addEventListener("touchend", (e) => {
    ...
    /* Keyboard — release at boundaries */
