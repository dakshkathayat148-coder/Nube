...
(function ($) {
    ...
    ////////////////////////////////////////////////////
    // 08. mobile menu Js
    var tpMenuWrap = $('.at-mobile-menu-active > ul').first().clone();
    if (!tpMenuWrap.length) {
        tpMenuWrap = $('.at-main-menu nav > ul').first().clone();
    }

    if (tpMenuWrap.length) {
        tpMenuWrap.find('.at-megamenu').each(function () {
            var $mega = $(this);
            var $flatSubmenu = $('<ul class="at-submenu submenu"></ul>');
            $mega.find('a').each(function () {
                var $link = $(this).clone();
                if ($link.attr('href')) {
                    $flatSubmenu.append($('<li></li>').append($link));
                }
            });
            if ($flatSubmenu.children().length) {
                $mega.replaceWith($flatSubmenu);
            }
        });

        var tpSideMenu = $('.at-offcanvas-menu nav');
        tpSideMenu.empty().append(tpMenuWrap.clone());

        tpSideMenu.find('li').has('> .at-submenu').each(function () {
            var $li = $(this);
            if (!$li.children('button.at-menu-close').length) {
                $li.append('<button class="at-menu-close" type="button" aria-label="Toggle submenu"><i class="fa-solid fa-plus"></i></button>');
            }
        });
    }

    $(document).on('click', '.at-offcanvas-menu nav li > button.at-menu-close', function (e) {
        e.preventDefault();
        var $parent = $(this).parent('li');
        var $submenu = $parent.children('.at-submenu');
        if (!$submenu.length) {
            return;
        }
    ...
    $(document).on('click', '.at-offcanvas-menu nav a', function () {
        $main_wrap.removeClass('menu-open');
        $openBtn.removeClass('active');
        $closeBtn.removeClass('active');
    });
    ...
    if ($('.slider-testimonial-thumbs').length > 0) {
        ...
        // Dynamic GSAP thumb animation — images reach viewport edges at any width
        mmThumbs.add('(min-width: 576px)', () => {
            const calcX = () => {
                const vw = window.innerWidth;
                const edgeDist = Math.max(vw / 2 - 275, 120);
                return {
                    t1: -edgeDist,
                    t2: -Math.max(edgeDist * 0.4, 60),
                    t4: Math.max(edgeDist * 0.6, 60),
                    t5: edgeDist,
                };
            };
            const tl = gsap.timeline({
            ...
            tl.to(thumb1, { x: () => calcX().t1, y: 200, duration: 1 }, 0)
              .to(thumb2, { x: () => calcX().t2, y: -100, duration: 1 }, 0)
              .to(thumb4, { x: () => calcX().t4, y: -70, duration: 1 }, 0)
              .to(thumb5, { x: () => calcX().t5, y: 200, duration: 1 }, 0);
            return () => tl.kill();
        });
    }
    ////////////////////////////////////////////////////
    ...
    function initScroll(section, items, direction) {
        ...
        const timeline = gsap.timeline({
            scrollTrigger: {
                ...
                pin: true,
                start: 'top top',
                end: () => `+=${items.length * (+section.dataset.scrollSpeed || 50)}%`,
                scrub: 1,
                invalidateOnRefresh: true,
                onLeave: () => { wrapper.style.height = '0'; wrapper.style.overflow = 'hidden'; },
                onEnterBack: () => { wrapper.style.height = ''; wrapper.style.overflow = ''; },
            },
            defaults: { ease: 'none' },
        });
        items.forEach((item, index) => {
